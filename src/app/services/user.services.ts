...
export class UserService {
  ...
  delete(userId) {
    ...
    return this._http
      .delete(this.url + "usuarios/delete/" + userId, { headers })
      .pipe(
        map((res: any) => {
          return res;
        }),
  ...
  obtenerTareas(userId) {
    return this._http.get(this.url + "tareas/"+userId)
      .map((res: any) => {
        return res;
      });
  }
  ...
  registrarTarea(tarea) {
    console.log("Se registra tarea.");
    console.log(tarea);
    const body = JSON.stringify(tarea);
    const headers = new HttpHeaders({
      "Content-Type": "application/json"
    });
    return this._http.post(this.url + "tareas/create", body, { headers }).pipe(
      map((res: any) => {
        return res;
      }),
      catchError(error => {
